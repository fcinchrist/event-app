...
4. [`supabase/migrations/004_event_users_extended.sql`](./supabase/migrations/004_event_users_extended.sql) — adds `user_status` and `member_type` columns to `event_users` (with safe `DEFAULT 'active'` / `'internal'` at the DB level for back-fill, and a `CHECK` constraint, so existing rows get a valid value automatically). The application sends `'external'` explicitly for new public registrations, and admins can override either field from the dashboard. Also adds an `event_users_insert_public` RLS policy so the public booking flow can create a new `event_users` row when an unknown phone number registers; UPDATE/DELETE remain admin-only via the existing `event_users_write_admin` policy.
