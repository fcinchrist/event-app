...
watch(
  ...
  ([open, user]) => {
    if (open && user) {
      ...
      // Modal dibuka tanpa user yang valid — reset form.
...
async function onSubmit(): Promise<void> {
  localError.value = null
  if (!props.user) {
    localError.value = 'User tidak ditemukan.'
    return
  }
  if (!canSubmit.value) {
  ...
  const result = await store.updateUser(props.user.id, {
  ...
  if (!result.success || !result.user) {
    localError.value = result.error ?? 'Gagal memperbarui user.'
    return
  }
  emit('updated', result.user.id)
...
<template>
  <UiAppModal
    ...
    subtitle="Perbarui data user. Perubahan langsung tersinkron ke semua event yang pernah diikuti."
    max-width="max-w-lg"
    :loading="store.isSubmitting"
  >
    <form class="p-6 space-y-4" @submit.prevent="onSubmit">
      <div v-if="props.user" class="text-[11px] text-slate-400 font-mono">
        ID: {{ props.user.id }}
      </div>
      ...
      <UiFormField
        ...
        hint="Akan dinormalisasi ke format 08… otomatis."
        required
      />
